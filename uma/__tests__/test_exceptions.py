...
def test_invalid_request_exception():
    ...
    assert json_output["status"] == "ERROR"
    assert json_output["reason"] == "Invalid request"
    assert json_output["code"] == "SYSTEM_ERROR"

    exc = InvalidRequestException("Custom reason", ErrorCode.INVALID_SIGNATURE)
    json_output = json.loads(exc.to_json())

    assert json_output["reason"] == "Custom reason"
